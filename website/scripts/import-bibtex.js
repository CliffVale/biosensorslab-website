...
function importBibtex() {
  ...
  parsed.forEach(entry => {
    ...
    const OUTPUT_DIR = PUBLICATIONS_DIR;
    ...
    // Venue fallback chain for non-journal entries
    ...
    // Cover image: check 'cover', 'image', 'figure' — OPTIONAL (omit when absent)
    let cover = cleanString(tags.cover || tags.image || tags.figure || '');

    // Validate cover image existence; drop the field entirely if the file is missing
    if (cover) {
      const relativeToRoot = cover.replace('../../', 'src/');
      const absolutePath = path.join(process.cwd(), relativeToRoot);

      if (!fs.existsSync(absolutePath)) {
        console.warn(`Warning: Cover image not found at ${absolutePath}. Omitting cover.`);
        cover = '';
      }
    }
